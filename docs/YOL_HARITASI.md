...
## Şimdi
- **Faz 1 — kod borcu** (aktif; ayrıntı TASK.md).
- **Faz 2.1 — demo akışı denetimi:** Faz 1 bitince başlar.
...
## Faz 1 — Kod borcu
Açık hata raporu maddelerini doğrula ve kapat (TASK.md). Çıkış: rapor boş, tam checker + frontend testleri yeşil. `engine.py:292` kararı Arda'dan gelir.
...
2. **2.2 Ekonomik metrik seti v1:** net PnL, max drawdown, işlem sayısı, ortalama giriş, toplam ücret, pozisyonda kalma süresi. Hesap çekirdekte, UI göstermekle yetinir.
3. **2.3 Reproduce + compare:** kayıtlı run'ı aynı config ve veriyle yeniden çalıştır, sonuç hash'i eşit çıksın; iki run yan yana karşılaştırılsın.
4. **2.4 Etkileşimli marker:** grafikte olay → hesap kaydı bağlantısı.
5. **2.5 Stress modeli:** yalnız 2.1–2.4 bittikten sonra, sınırlı tek dilim.
