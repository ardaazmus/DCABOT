...
6. **3.6 Dolum + restart kurtarma (kod tamam, offline doğrulandı; REAL_TESTNET kanıtı Arda'yı bekliyor):** `AttemptStore.recover_after_restart` artık yalnız `SENDING` değil `PREPARED`/`PERSISTED`'i de kurtarıyor (bulundu: `can_transition` bu ikisine `UNKNOWN`'a çıkış vermiyordu — sonsuz kilitlenme riski, düzeltildi). Yeni `application/testnet_order_execution.py::recover_stuck_attempts` — her oturum başında (`tools/run_single_testnet_order.py`'ye bağlandı) kalan attempt'leri gerçek sorguyla çözer. Ayrıntı: docs/KARARLAR.md.
