...
| F22 | Kaydedilmiş koşu, kıyas ve log–chart bağlantısı | C3 | Aynı dataset/model; olaydan chart noktasına ulaşım; export | P1.06.d/e list/detail + Faz 2.3 `POST /api/historical-runs/{run_id}/reproduce` (hash karşılaştırmalı, fail-closed) + `SavedRunsPanel.tsx` yan yana kıyas + Faz 2.4 grafik↔tablo çift yönlü marker `COMPLETE_WITH_LIMITATION`; yalnız CSV/JSON export `PLAN` (2026-09-21 düzeltme: önceki `PLAN` kaydı stale idi, bağımsız kod incelemesiyle doğrulandı) |
